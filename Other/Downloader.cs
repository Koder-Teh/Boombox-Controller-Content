...
{
    internal class Downloader : BoomboxController
    {
        ...
        public static bool Unpacking()
        {
            ...
            {
                plugin.Log("Zip файл поврежден, потому что он не был загружен должным образом. Сложно ли не закрывать игру и просто позволить загрузке файлов завершиться?");
                plugin.Log("Zip is broken because it wasn't downloaded properly. Is it difficult to not close the game and just let the downloading of files to finish?");
                File.Delete(@"BoomboxController\other\ffmpeg-master-latest-win64-gpl.zip");
                return false;
            }
        }
